...
import { runAddChatMembers, runCreateChat, runDeleteChat, runGetChat, runListChatMembers, runListChats, runRemoveChatMembers, runRenameChat } from "./service.js";
...
server.tool("rename_chat", "修改飞书群名称。", {
...
server.tool("delete_chat", "解散飞书群聊。", {
    chat_id: z.string().min(1).describe("Chat ID, starts with oc_."),
    timeout: z.number().int().min(1).max(120).optional().describe("Optional timeout in seconds.")
...
server.tool("add_chat_members", "向飞书群聊添加成员。", {
    chat_id: z.string().min(1).describe("Chat ID, starts with oc_."),
...
server.tool("remove_chat_members", "从飞书群聊移除成员。", {
    chat_id: z.string().min(1).describe("Chat ID, starts with oc_."),
...
server.tool("list_chats", "获取飞书群列表。", {
...
server.tool("list_chat_members", "获取飞书群成员列表。", {
    chat_id: z.string().min(1).describe("Chat ID, starts with oc_."),
    page_size: z.number().int().min(1).max(100).optional().describe("Optional page size. Defaults to 50."),
    page_token: z.string().optional().describe("Optional page token for next page."),
    timeout: z.number().int().min(1).max(120).optional().describe("Optional timeout in seconds.")
