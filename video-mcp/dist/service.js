...
function toInteger(value, defaultValue, min) {
...
export function resolveApiVariant(model) {
...
async function getUploadPolicy(config, model) {
...
async function uploadBinaryAsOssUrl(config, model, asset) {
...
async function resolveInputAssetToUrl(config, model, rawValue) {
...
function buildModernParameters(input) {
...
function buildModernMedia(firstFrameUrl, lastFrameUrl) {
...
async function createLegacyTask(config, input, resolved) {
...
async function createModernTask(config, input, resolved) {
...
async function waitTaskToComplete(config, model, apiVariant, inputAssets, inputOptions, createResult, timeoutMs, pollIntervalMs) {
...
export async function generateVideoFromFrames(input) {
    ...
    const createResult = apiVariant === "legacy_kf2v"
    ...
    return waitTaskToComplete(config, model, apiVariant, { first_frame: firstFrameUrl, last_frame: lastFrameUrl }, input, createResult, timeoutMs, pollIntervalMs);
}
export async function generateVideoFromFirstFrame(input) {
    const config = resolveVideoConfig();
    const model = input.model ?? config.model;
    const apiVariant = resolveApiVariant(model);
