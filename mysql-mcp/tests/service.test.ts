...
function serviceWith(client: FakeClient, database = "app"): MysqlService {
  return new MysqlService(client, { maxRows: 2, database });
}
...
test("query uses bound client", async () => {
  const client = new FakeClient([{ rows: [{ id: 2 }], fields: [{ name: "id" }] }]);
  const service = serviceWith(client);

  assert.deepEqual(await service.query({ sql: "select id from events" }), {
    rows: [{ id: 2 }],
    fields: ["id"],
    row_count: 1,
    truncated: false
  });
  assert.equal(client.calls[0]?.sql, "select id from events LIMIT ?");
});
