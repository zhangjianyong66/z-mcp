...
function resolveConfig(modelOverride?: string): { apiKey: string; baseURL: string; model: string } {
  ...
  return {
    ...
    model: modelOverride ?? readEnv("DASHSCOPE_MODEL", "LLM_MODEL") ?? DEFAULT_MODEL
  };
}
...
async function generateImage(input: {
  ...
  const config = resolveConfig(input.model);
...
server.tool(
  ...
  "Generate images with DashScope qwen-image and return temporary image URLs.",
  {
    ...
    model: z.string().min(1).optional().describe("Optional DashScope model override."),
    size: z.string().min(1).optional().describe("Optional output size, such as 1024*1024."),
    n: z.number().int().min(1).max(4).optional().describe("Optional number of images to generate."),
  ...
  async ({ prompt, model, size, n, negative_prompt, watermark }) => {
    try {
      const result = await generateImage({ prompt, model, size, n, negative_prompt, watermark });
      return {
        content: [
          {
            type: "text",
