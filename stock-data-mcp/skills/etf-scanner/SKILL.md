...
本技能采用 MCP-first 模式：
- 候选池固定来自 `references/etf_universe.md`
...
主流程工具：
1. `stock-data__etf_batch_decide`
...
2. 创建目录：`artifacts/etf-scanner/<run_id>/`。
3. 初始化 `manifest.json`。
4. 初始化 `debug.enabled=false`。
5. 若用户请求命中 debug 关键词（`记录debug日志`、`开启调试日志`、`保留排障日志`），设置 `debug.enabled=true`，创建 `artifacts/etf-scanner/<run_id>/debug/`，并在 `manifest.json` 回填 `debug.logPath`。
...
1. 读取 `references/etf_universe.md`。
2. 仅以该文件作为唯一真源，提取当次 symbol 列表（禁止外部补充、替换、推断）。
3. 标准化 symbol（`SH|SZ+6位` -> 裸码）并按出现顺序去重。
4. 按顺序分批（每批 `<=20`）生成 `planned_batches`。
5. 写入 `input.json`（至少包含 `universeSourcePath/universeCount/universeSymbolsNormalized/plannedBatches`）。
...
1. 候选、观察名单、不执行清单全部依据 MCP 返回结果投影。
2. 候选上限、观察区间、闸门状态均以 MCP 字段为准。
3. 写入 `scoring.json`。

### Step 6: Render

1. 渲染动作字典：`open_buy`、`increase_buy`、`replace_buy`、`hold_watch`、`no_trade`。
2. `action` 展示统一映射为 `raw_action（中文）`，中文标签映射：
   - `open_buy` -> `新开买入`
   - `increase_buy` -> `加仓买入`
   - `replace_buy` -> `换仓买入`
   - `hold_watch` -> `持有观察`
...
3. 仅引用 MCP 返回参数：`entryPrice/stopLoss/targetQty/deltaQty`。
4. 仅引用 MCP 返回暴露字段：`symbolExposure/symbolCap/symbolRatio/symbolExposureRoom/symbolExposureQty`。
5. `actionReason` 展示统一映射为 `raw_code（中文）`，中文标签映射：
   ...
   - 未命中映射 -> `raw_code（未定义中文标签）`
6. 强制生成 `可执行交易单（仅含 open_buy/increase_buy/replace_buy）` 区块。
7. 仅过滤 `results[].action in {open_buy,increase_buy,replace_buy}`，不得追加策略条件。
8. 若过滤结果为空，固定输出：`本批无可执行交易单（0条）`。
9. 若过滤结果非空，逐条映射字段：`代码/动作(action，编码+中文)/entryPrice/stopLoss/targetQty/deltaQty/actionReason(编码+中文)`。
10. 映射顺序沿用 MCP 返回顺序，不做二次排序与数值重格式化。
11. 候选/观察/不执行三类区块中，每个 ETF 必须追加一行结构信息：
    `趋势：trend（trendZh）；结构：通过|不通过（structureReasonZh）；结构快照：price/MA5/MA10/MA20, 偏离MA10=priceVsMa10Pct%；安全边际：safetyMarginPct%（阈值4.00%）`。
12. `insufficient_safety_margin` 必须使用固定格式：
    `安全边际不足（当前 {safetyMarginPct}% < 阈值 4.00%，差 {gapPct}%）`；
    其中 `gapPct=max(0,4.00-safetyMarginPct)`，保留 2 位小数。
13. 同一 ETF 命中多原因时，`insufficient_safety_margin` 文案优先展示在第一条，其余原因按原顺序追加。

### Step 7: Finalize

1. 非中止场景生成六段完整报告正文（含 `可执行交易单`）。
...
7. 归因中文化仅为展示层映射，禁止改写或覆盖 MCP 原始 `actionReason`。
8. 动作中文化仅为展示层映射，禁止改写或覆盖 MCP 原始 `action`。
9. 若 `final_output.md` 存在，聊天回复必须与其严格全文一致。
10. 若聊天缺失完整报告，则本次运行判定失败，即使文件齐全也失败。
11. Universe 唯一真源必须是 `references/etf_universe.md`，禁止在主流程中使用外部 symbol 源。
...
记录范围（开启后强制）：
- 主流程工具：`stock-data__etf_batch_decide`
...
- ETF 池：`references/etf_universe.md`
